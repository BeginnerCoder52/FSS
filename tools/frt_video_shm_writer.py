#!/usr/bin/env python3
"""
Stream a video file into the FRTApp POSIX shared-memory frame buffer.

This is a demo replacement for cpp_camera_core. It writes the same 64-byte
header + JPEG payload consumed by frt_app/py_ai_core/src/ShmReader.py.
"""
...
def parse_args():
    ...
    parser.add_argument("--fps", type=float, default=0.0,
                        help="Playback FPS. Default: source FPS, fallback 30.")
    parser.add_argument("--max-seconds", type=float, default=0.0,
                        help="Stop after this many playback seconds. Default: full video.")
    ...
    parser.add_argument("--loop", action="store_true",
                        help="Loop the video until interrupted or --max-seconds expires.")
...
def write_frame(shm, frame, frame_id: int, quality: int, shm_size: int):
    ...
    # Commit frame data first, then header with the new frame_id.
...
def main() -> int:
    ...
    max_frames = int(args.max_seconds * playback_fps) if args.max_seconds > 0 else 0
    ...
    print(
        "SHM writer ready: "
        f"path={args.shm_path}, video={args.video}, fps={playback_fps:.2f}, "
        f"max_seconds={args.max_seconds if args.max_seconds > 0 else 'full'}",
        flush=True,
    )
