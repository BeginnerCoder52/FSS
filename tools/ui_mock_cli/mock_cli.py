...
try:
    import sdbus
    from sdbus import DbusInterfaceCommonAsync, dbus_signal_async, DbusInterfaceCommon, dbus_method
except ImportError:
...
# RecommendDaemon proxy (reads from real daemon if running)
class RecommendDaemonProxy(DbusInterfaceCommon, interface_name="vn.edu.uit.FSS.RecommendDaemon"):
    @dbus_method('ss', 's')
...
# ======================================================================
# 3. RecommendDaemon Search (Real D-Bus or fallback)
# ======================================================================
def search_recipe(recipe_name: str):
    """Call RecommendDaemon.GenerateShoppingList via D-Bus and print result."""
    print(f"\n[TÌM KIẾM] Đang tìm công thức: '{recipe_name}'")
    try:
        proxy = RecommendDaemonProxy(
            "vn.edu.uit.FSS.RecommendDaemon",
            "/vn/edu/uit/FSS/RecommendDaemon"
        )
        batch_id = str(uuid.uuid4())
        raw = proxy.GenerateShoppingList(recipe_name, batch_id)
        result = json.loads(raw)
        status = result.get("status", "?")
        print(f" -> [D-Bus] Trạng thái: {status}")
        if status == "SUCCESS":
            print(f"    Món ăn  : {result.get('recipe_name')}")
            print(f"    Pipeline: {result.get('pipeline_time_ms')} ms")
            print(f"    Tổng NL : {result.get('total_items')} | Có: {result.get('available_count')} | Cần thêm: {result.get('needed_count')} | Thiếu: {result.get('missing_count')}")
            print("    --- NGUYÊN LIỆU CẦN MUA ---")
            for cat, label in [("needed", "CẦN THÊM"), ("missing", "THIẾU HẲN")]:
                items = result.get(cat, [])
                for item in items:
                    food = item.get("food_id", item.get("name", "?"))
                    req = item.get("required_qty", "?")
                    avail = item.get("available_qty", 0)
                    short = item.get("shortage", "?")
                    unit = item.get("unit", "")
                    print(f"      [{label}] {food}: cần {req}{(' ' + unit) if unit else ''}, có {avail}, thiếu {short}{(' ' + unit) if unit else ''}")
        elif status == "NOT_FOUND":
            print(f"    Không tìm thấy công thức: {result.get('message', '')}")
        else:
            print(f"    Lỗi: {result.get('error', raw)}")
        print(f"\n    Full JSON:\n{json.dumps(result, ensure_ascii=False, indent=4)}\n")
    except Exception as e:
        print(f" -> [Lỗi D-Bus] Không kết nối được RecommendDaemon: {e}")
        print("    Gợi ý: source recommend_daemon/venv/bin/activate && python recommend_daemon/src/main.py")


def list_available_recipes():
    """Call RecommendDaemon.GetAvailableRecipes and print list."""
    print("\n[DANH SÁCH] Đang lấy danh sách công thức...")
    try:
        proxy = RecommendDaemonProxy(
            "vn.edu.uit.FSS.RecommendDaemon",
            "/vn/edu/uit/FSS/RecommendDaemon"
        )
        raw = proxy.GetAvailableRecipes()
        recipes = json.loads(raw)
        print(f" -> Có {len(recipes)} công thức khả dụng:")
        for i, r in enumerate(recipes[:20], 1):
            print(f"    {i:2d}. {r}")
        if len(recipes) > 20:
            print(f"    ... và {len(recipes) - 20} công thức khác.")
    except Exception as e:
        print(f" -> [Lỗi D-Bus] {e}")
...
def print_menu():
    print("=" * 55)
    print("  FSS Mock CLI  (Môi trường Test Độc lập)")
    print("=" * 55)
    print("--- Kho thực phẩm (FRT / D-Bus) ---")
    ...
    print("--- Gợi ý thực đơn (RecommendDaemon) ---")
    print("  7. Tìm kiếm công thức (nhập tên món)")
    print("  8. Liệt kê tất cả công thức có sẵn")
    print("-" * 55)
    print("  0. Thoát")
    print("=" * 55)
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            try:
                recipe = input("Nhập tên món ăn: ").strip()
                if recipe:
