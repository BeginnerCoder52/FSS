...
def test_camerauvcdriver():
    ...
    try:
        ...
        # Test 2: check_uvc_connection() - Device check
        try:
            result = driver.check_uvc_connection()
            assert isinstance(result, bool)
            if not result:
                results.add_warn("CameraUvcDriver.check_uvc_connection()", 
                               "/dev/video0 not found (expected in test env)")
            else:
                results.add_pass("CameraUvcDriver.check_uvc_connection()")
        except Exception as e:
            results.add_fail("CameraUvcDriver.check_uvc_connection()", str(e))
        
        # Test 3: open_camera_stream() - Will likely fail in test env
        try:
            result = driver.open_camera_stream()
            assert isinstance(result, bool)
            if not result:
                results.add_warn("CameraUvcDriver.open_camera_stream()",
                               "Camera device not available (expected in test env)")
            else:
                results.add_pass("CameraUvcDriver.open_camera_stream()")
        except Exception as e:
            results.add_fail("CameraUvcDriver.open_camera_stream()", str(e))
        
        # Test 4: read_frame() signature
        try:
            frame = driver.read_frame()
            # Should return None if camera not open
            results.add_pass("CameraUvcDriver.read_frame() -> ndarray|None")
        except Exception as e:
            results.add_fail("CameraUvcDriver.read_frame()", str(e))
        
        # Test 5: release_camera() signature
        try:
...
def test_yolotfliteengine():
    ...
    try:
        ...
        # Test 2: load_model_mmap()
        try:
            result = engine.load_model_mmap()
            assert isinstance(result, bool)
            if not result:
                results.add_warn("YoloTfliteEngine.load_model_mmap()",
                               "Model file not found (expected - download model to /opt/fss/models/)")
            else:
        ...
        # Test 6: get_output_boxes()
        try:
            boxes = engine.get_output_boxes()
            assert isinstance(boxes, list)
            results.add_pass("YoloTfliteEngine.get_output_boxes() -> list")
        except Exception as e:
