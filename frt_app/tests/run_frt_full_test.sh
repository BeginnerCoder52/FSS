...
if [[ "$MODE" == "auto" ]]; then
    ...
    echo ""
    echo "  ╔══════════════════════════════════════════════════════════════════╗"
    echo "  ║  FRTAPP 2-LINE CHECK-IN/CHECK-OUT MODE                        ║"
    echo "  ║  Door sensor is BYPASSED — pipeline starts immediately.        ║"
    echo "  ║  2 fixed virtual lines divide the frame into 3 zones:          ║"
    echo "  ║    • Line A (outer, ~33%): threshold entry                     ║"
    echo "  ║    • Line B (inner, ~66%): inside fridge                       ║"
    echo "  ║    • Cross A THEN B downward = CHECK_IN  (object enters)       ║"
    echo "  ║    • Cross B THEN A upward  = CHECK_OUT (object leaves)        ║"
    echo "  ║                                                               ║"
    echo "  ║  HOW TO TEST:                                                  ║"
    echo "  ║  1. Hold any object (hand, fruit, can) in front of camera      ║"
    echo "  ║  2. Move it vertically across the frame (top → bottom)         ║"
